...
const state = { quickShortcut: '', annotateShortcut: '', openAtLogin: false };
...
const openAtLoginRow = openAtLoginEl.closest('.rowToggle');
...
async function load() {
  ...
  state.openAtLogin = !!s.openAtLogin;
  renderShortcut(quickEl, state.quickShortcut);
  renderShortcut(annEl, state.annotateShortcut);
...
openAtLoginEl.addEventListener('change', () => {
  state.openAtLogin = openAtLoginEl.checked;
});

openAtLoginRow.addEventListener('click', (e) => {
  if (e.target.closest('.toggle')) return;
  openAtLoginEl.checked = !openAtLoginEl.checked;
  state.openAtLogin = openAtLoginEl.checked;
});
